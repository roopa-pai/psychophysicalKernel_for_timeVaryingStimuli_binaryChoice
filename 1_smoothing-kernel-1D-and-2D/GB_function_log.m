%CV 1D
%tau (length scale) range in log scale, round 1
clearvars
...
folder = '2 - CV 1D both log, -2 to 2, 100 reps';
figfolder = strcat('C:\Users\roopa\Documents\caesardata\18.1.5\',folder,'\');
mkdir (figfolder);
...
for mat_i = 1:matsize
    ...
    for i = 1:ls
        ...
        %map of log likelihood to find best st value pair.
        sigmas = -2:2;
        taus1 = -2:2;
        ...
        for index_s = 1:length(sigmas)
            ...
            for index_t1 = 1:length(taus1)
                tau1 = taus1(index_t1);
                [L_STM, ws_STM] = CVfutn (st(i).stim, st(i).dec, 10^sigma, 10^tau1);
        ...
        set(gca,'YDir','normal')
        title(['Log likelihood (',num2str(strength(i)),') = ',num2str(st(i).opt_L),'; sigma = log_1_0(',num2str(st(i).opt_sigma),'), tau = log_1_0(', num2str(st(i).opt_tau1),')']);
        xlabel ('tau (log_1_0)')
        ylabel ('sigma (log_1_0)')
        colorbar;
